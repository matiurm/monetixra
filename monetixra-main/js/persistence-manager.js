...
class PersistenceManager {
  ...
  async clearAllData() {
    ...
    // Keep a rescue snapshot outside the normal persistence prefix so a reset can be recovered.
    try {
      if (typeof window !== 'undefined' && window.D) {
        const rescuePayload = JSON.stringify(window.D);
        localStorage.setItem('mxt_rescue_snapshot', rescuePayload);
        localStorage.setItem('mxt_rescue_session', window.D.cur || '');
        sessionStorage.setItem('mxt_rescue_snapshot', rescuePayload);
        sessionStorage.setItem('mxt_rescue_session', window.D.cur || '');
      }
    } catch (e) {
      console.warn('[PersistenceManager] Rescue snapshot save failed:', e);
    }
